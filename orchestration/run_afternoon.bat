@echo off
setlocal

REM PROJECT_ROOT resolved dynamically from script location
set "PROJECT_ROOT=%~dp0.."
if "%PROJECT_ROOT:~-1%"=="\" set "PROJECT_ROOT=%PROJECT_ROOT:~0,-1%"
set "LOG=%PROJECT_ROOT%\phases\02_curate\logs\run.log"

REM Add Anaconda and Python locations to PATH dynamically
if exist "%USERPROFILE%\AppData\Local\anaconda3" set "PATH=%USERPROFILE%\AppData\Local\anaconda3;%USERPROFILE%\AppData\Local\anaconda3\Scripts;%USERPROFILE%\AppData\Local\anaconda3\Library\bin;%PATH%"
if exist "%USERPROFILE%\AppData\Local\Python" set "PATH=%USERPROFILE%\AppData\Local\Python\bin;%USERPROFILE%\AppData\Local\Python;%PATH%"

REM Idempotency guard — derive YYYYMMDD via wmic (locale-independent)
for /f "tokens=2 delims==" %%I in ('wmic os get localdatetime /value 2^>nul') do set _DT=%%I
set TODAY=%_DT:~0,8%
set "DONE_FLAG=%PROJECT_ROOT%\phases\02_curate\logs\afternoon_%TODAY%.done"

if exist "%DONE_FLAG%" (
    echo [%DATE% %TIME%] SKIP: afternoon upload already completed today ^(sentinel: %DONE_FLAG%^). >> "%LOG%"
    echo [%DATE% %TIME%] Delete the sentinel file to force a re-run. >> "%LOG%"
    exit /b 0
)

echo [%DATE% %TIME%] === Afternoon Run Started === >> "%LOG%"

REM Step 1: Run the Python news scraper
echo [%DATE% %TIME%] Running scraper... >> "%LOG%"
cd /d "%PROJECT_ROOT%\phases\01_scrape"
call run.bat >> "%LOG%" 2>&1
if errorlevel 1 (
    echo [%DATE% %TIME%] ERROR: Scraper failed. >> "%LOG%"
    exit /b 1
)
echo [%DATE% %TIME%] Scraper finished. >> "%LOG%"

REM Step 2: Run HSX insider trading collection for the afternoon report.
REM Afternoon run at 15:00 covers the last 7 hours from the morning boundary.
echo [%DATE% %TIME%] Running HSX insider trading pipeline... >> "%LOG%"
cd /d "%PROJECT_ROOT%\phases\01_scrape"
"%PROJECT_ROOT%\phases\01_scrape\venv\Scripts\python.exe" hsx_insider_scraper.py --session afternoon >> "%LOG%" 2>&1
if errorlevel 1 (
    echo [%DATE% %TIME%] WARNING: HSX insider scraper failed. Continuing news pipeline. >> "%LOG%"
) else (
    echo [%DATE% %TIME%] Preparing HSX PDFs and images... >> "%LOG%"
    "%PROJECT_ROOT%\phases\01_scrape\venv\Scripts\python.exe" hsx_prepare_pdfs.py >> "%LOG%" 2>&1
    if errorlevel 1 (
        echo [%DATE% %TIME%] WARNING: HSX PDF/image preparation failed. >> "%LOG%"
    )
    for /f "delims=" %%F in ('dir /b /o-d hsx_insider_trading_*_extracted.json 2^>nul') do (
        "%PROJECT_ROOT%\phases\01_scrape\venv\Scripts\python.exe" format_hsx_trading_news.py "%%F" >> "%LOG%" 2>&1
        goto :afternoon_hsx_done
    )
)
:afternoon_hsx_done

REM Step 3: Upload to Google Sheets
echo [%DATE% %TIME%] Uploading to Sheets... >> "%LOG%"
cd /d "%PROJECT_ROOT%\phases\02_curate"
node upload-to-sheets.js afternoon >> "%LOG%" 2>&1
if errorlevel 1 (
    echo [%DATE% %TIME%] ERROR: Upload failed. >> "%LOG%"
    exit /b 1
)

REM Write sentinel so re-runs are blocked for the rest of the day
echo done > "%DONE_FLAG%"

echo [%DATE% %TIME%] === Afternoon Run Complete === >> "%LOG%"
endlocal

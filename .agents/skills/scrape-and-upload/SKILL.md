...
description: Run Vietnam financial news scraper, HSX insider trading pipeline, and upload CSV data to Google Sheets for curation.
---
...
The agent reads `phases\01_scrape\*_to_format.json`, translates each transaction into bilingual `summary_vn` and `summary_en` prose following KIS trading-item rules, and writes the result to `phases\01_scrape\*_agent_formatted.json`.

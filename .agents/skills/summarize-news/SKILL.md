...
2b. **Extract and Format HSX Insider Trading disclosures:**
   ...
   - **Case B: The manifest contains PDFs to extract**:
     1. Run the manifest text extractor:
        ```powershell
        phases\01_scrape\venv\Scripts\python.exe phases\01_scrape\extract_hsx_from_manifest.py --manifest phases\01_scrape\hsx_insider_trading_{timestamp}_manifest.json
        ```
        *(Note: **NEVER call `view_file` on `.png` image files in `pdf_images/`**. Calling `view_file` on large binary PNG images will exhaust context and crash the agent environment with `agent terminated due to errored`.)*
     2. Review and adjust `phases\01_scrape\hsx_insider_trading_{timestamp}_agent_results.json` if needed (reading text only).
     3. Merge the extracted transactions into `_extracted.json`:
        ```powershell
...
6. **Same-ticker / same-company overlap check (replaces deduplicate_reports.py):**
   After `combine_chunks.py` writes `{base}_corporate.json`, read the file and scan for duplicate ticker coverage before running harness:
   - If two or more items share the **same ticker** (non-empty, non-`Unlisted`/`OTC`/`N/A`), keep only the one with the longest `summary_vn`. Log the dropped item title.
   - If two items have **no valid ticker** but share ≥2 significant words in `company_vn` or `company_en` (exclude stopwords: `công`, `ty`, `cổ`, `phần`, `tập`, `đoàn`, `jsc`, `corporation`, `company`, `group`, `limited`, `vn`), and their summaries are substantively similar (covering the same corporate action), merge or drop the shorter one.
   - Write the cleaned JSON back to `{base}_corporate.json` and update `item_count`.
   - This step is agent-judgment only — do not run any external script. If unsure whether two items are truly duplicates, keep both.
7. After writing each JSON file, run the harness to validate output (see **Harness Validation** below).
8. When all items pass (or retries are exhausted), automatically invoke the `/dedup-news` skill.
...
- **Do NOT start with date**: Do NOT start summaries with date prefixes (e.g. do NOT start with `Ngày D/M,` or `On D Month,`). Start directly with the main subject/action.
